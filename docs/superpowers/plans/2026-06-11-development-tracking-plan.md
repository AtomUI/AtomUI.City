...
- 分支：`develop`
- 当前提交：`2de1a30 fix(Data): map gRPC transport failures`
- 当前测试：`dotnet build AtomUICity.slnx --no-restore` 通过；`bash engineering/test-ci.sh` 通过，580 tests passed
- 当前状态：已完成 Data HTTP send transport error mapping
