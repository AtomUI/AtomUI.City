...
- 分支：`develop`
- 当前提交：`d771de0 fix(Routing): protect route template collections`
- 当前测试：`dotnet build AtomUICity.slnx --no-restore` 通过；`bash engineering/test-ci.sh` 通过，616 tests passed；`dotnet format AtomUICity.slnx --verify-no-changes --no-restore` 通过；`bash engineering/check-docs.sh` 通过；`git diff --check` 通过
- 当前状态：已完成 Security policy requirements readonly collection
